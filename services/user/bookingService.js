...
const getProductDetails = (
  connection,
  { hair_color, smoothing_product, keratin_product }
) => {
  const queries = [];
...
const createBooking = async (data) => {
  ...
  try {
    ...
    // =================================================================
    const { layananWithCategory } = await validationService.validateBookingData(
      data,
      connection
    );
    ...
    try {
      ...
      await validationService.validateStock(
        hair_color,
        smoothing_product,
        keratin_product,
        connection
      ...
      const cancel_timer = Math.max(
        0,
        Math.floor(
          (new Date(`${tanggal}T${jam_mulai}+08:00`).getTime() +
            30 * 60000 -
            Date.now()) /
            1000
        )
      ...
      if (booking_id) {
        try {
          console.log(
            `[BookingService] Error terjadi, restore stok untuk booking_id=${booking_id}`
          );
          await stockService.restoreStockByBookingId(booking_id);
        } catch (restoreError) {
          console.error(
            `[BookingService] Gagal restore stok untuk booking_id=${booking_id}:`,
            restoreError
          );
        }
      }
...
const cancelBooking = async (id, user_id) => {
  ...
  try {
    ...
    try {
      console.log(
        `[BookingService] Restore stok untuk booking yang dibatalkan id=${id}`
      );
      await stockService.restoreStockByBookingId(id, connection);
    } catch (restoreError) {
      console.error(
        `[BookingService] Gagal restore stok untuk booking_id=${id}:`,
        restoreError
      );
