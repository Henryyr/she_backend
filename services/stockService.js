const updateHairColorStock = async (connection, hairColor) => {
    const [result] = await connection.query(
        'SELECT stock FROM hair_colors WHERE id = ?',
        [hairColor.color_id]
    );

    if (!result[0]) throw new Error('Stock hair color tidak ditemukan');
    ...
    await connection.query(
        'UPDATE hair_colors SET stock = stock - 1 WHERE id = ?',
        [hairColor.color_id]
    );
};
...
const updateSmoothingStock = async (connection, smoothingProduct) => {
    const [result] = await connection.query(
        'SELECT stock FROM smoothing_products WHERE id = ? AND brand_id = ?',
    ...
    // Deduct stock
    await connection.query(
        'UPDATE smoothing_products SET stock = stock - 1 WHERE id = ? AND brand_id = ?',
        [smoothingProduct.product_id, smoothingProduct.brand_id]
    );
};
...
const updateKeratinStock = async (connection, keratinProduct) => {
    const [result] = await connection.query(
        'SELECT stock FROM keratin_products WHERE id = ? AND brand_id = ?',
    ...
    // Deduct stock
    await connection.query(
        'UPDATE keratin_products SET stock = stock - 1 WHERE id = ? AND brand_id = ?',
        [keratinProduct.product_id, keratinProduct.brand_id]
    );
};
